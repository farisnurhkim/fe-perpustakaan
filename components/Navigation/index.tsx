...
const Navigation = ({ type }: { type: "admin" | "member" }) => {
    ...
    const { items } = useCart();
    const totalBuku = items.reduce((total, item) => {
            ...
            <div className='flex items-center gap-2 py-2 px-4 max-w-full overflow-x-auto'>
                {data.map((item, index) => (
                    <Link href={item.url} key={index} className={cn(
                        'py-1.5 px-4 w-44 md:w-55 gap-2 flex items-center justify-center rounded-3xl text-slate-950 transition-all relative',
                        ...
                        <div className='absolute right-1 -top-1 sm:top-1'>
                            {type === "member" && item.url === "/keranjang" &&totalBuku > 0 && (
                                <div className='bg-emerald-600 rounded-lg px-2.5 py-1 text-xs flex items-center justify-center text-white'>
                                    {totalBuku}
                                </div>
                            )}
                        </div>
